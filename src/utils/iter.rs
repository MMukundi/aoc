...
impl <I:Iterator> Iterator for Chunks<I>
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if take.is_empty() { None }else{ Some(take) }
    }
}
...
impl <I:Iterator> Iterator for StepBy<I>
{
    type Item=I::Item;
    fn next(&mut self) -> Option<Self::Item> {
        for _ in 0..(self.1-1) { self.0.next(); }
        self.0.next()
    }
}
...
impl <I:Iterator> Iterator for SkipEvery<I>
{
    type Item=I::Item;
    fn next(&mut self) -> Option<Self::Item> {
        match self.0.next().map(
            |(i,n)|
            if (i+1)%self.1==0 { self.0.next().map(|(_,n2)|n2) } else {Some(n)}
        ) {
            Some(x)=>x,
            None=> None
        }
...
pub trait IterAddons: Iterator {
    fn to_chunks(self,chunk_size:usize) -> Chunks<Self> where Self:Sized { Chunks(self,chunk_size) }
    fn skip_every(self, n:usize)-> SkipEvery<Self> where Self:Sized{ SkipEvery(self.enumerate(), n) }
    fn step(self, n:usize)-> StepBy<Self> where Self:Sized{ StepBy(self, n) }
}
