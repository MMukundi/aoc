...
impl Board{
    fn update(&mut self,ball:&usize,i:usize){
        if self.2.is_none(){
            self.0.iter().position(|b|b==ball).map(|i|self.1[i]=true);
            self.2 = if self.has_won(){Some((i,*ball))}else{None}
        }
    }
    ...
    fn has_won(&self)->bool{
        (0usize..25).to_chunks(5).any(|i|self.all_true(&mut i.into_iter()))||
        (0usize..5).map(|c|(0usize..25).skip(c).step_by(5)).any(|i|self.all_true(&mut i.into_iter()))
    }
    fn score(&self)->usize{
        self.0.iter().zip(self.1.iter()).filter(|(_,b)|!**b).map(|(n,_)|n).sum()
    }
...
impl FromStr for Board{
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let nums = s.split_whitespace().map(|x|x.parse::<usize>().unwrap()).collect::<Vec<usize>>();
        Ok(Board(nums,vec![false;25],None))
    }
}

pub struct Day4Solution{sorted_boards:Vec<Board>}
impl Solution for Day4Solution{
    ...
    fn solve_second_star(&mut self)->Self::Answer {
        self.sorted_boards.last().map(|b|b.win_score()).unwrap_or_default().unwrap_or_default()
    }
    fn solve(input:String)->Day4Solution{
        let mut lines = input.split("\n\n");
        let balls = lines.next().unwrap().split(",").map(|n|n.parse::<usize>().unwrap()).collect::<Vec<usize>>();
        ...
        Day4Solution{sorted_boards: boards}
    }
}
