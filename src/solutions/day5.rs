...
impl FromStr for Line {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut points = s
            .split(" -> ")
            .map(|point_string| point_string.parse::<Point>().unwrap());
...
impl Day5Solution {
    ...
    fn count_multiple_overlaps<'a>(&self, lines: impl Iterator<Item = &'a Line>) -> usize {
        ...
        for line in lines {
            line.iter()
                .for_each(|Point { x, y }| self.index(x, y).into_iter().for_each(|i| map[i] += 1));
        }
...
impl Solution for Day5Solution {
    ...
    fn solve(input: String) -> Day5Solution {
        ...
        Day5Solution {
            board_size: (board_bounds.x * board_bounds.y).try_into().unwrap_or(0),
            lines,
            board_bounds,
        }
    }
