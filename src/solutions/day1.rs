...
pub struct Day1Solution{heights:Vec<u32>}
impl Solution for Day1Solution{
    type Answer=usize;

    fn num()->usize { 1 }
    fn units() ->String { "increases".to_string() }
    ...
    fn solve(input:String)->Day1Solution{
        Day1Solution{heights:input.split_whitespace().map(|line|line.parse::<u32>().unwrap()).collect::<Vec<u32>>()}
    }
}
