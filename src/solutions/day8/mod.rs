...
impl Solution for Day8Solution{
    ...
    fn num()->usize { 8 }
    fn units() ->String { "".to_string() }
    fn solve_first_star(&mut self)->Self::Answer {
        self.0.iter().map(|e|e.outputs.iter().filter(|o|match o.count_active(){2|3|4|7=>true,_=>false}).count()).sum()
    }

    fn solve_second_star(&mut self)->Self::Answer {
        let actuals = (0..10).map(|i|Entry::segments_for(i)).collect::<Vec<Segments>>();
        let digit_map = actuals.iter().enumerate().map(|(i,s)|(*s,i)).collect::<HashMap<Segments,usize>>();
        self.0.iter_mut().map(|e|{
                ...
                let map = e.could_be.iter().enumerate().map(|(i,s)|(*s,Segments::from(1<<i))).collect::<HashMap<Segments,Segments>>();
                let output = e.outputs.iter().map(|out|{
    ...
    fn solve(input:String)->Self {
        Day8Solution(input.lines().map(|line|line.parse::<Entry>().unwrap()).collect())
    }
}
