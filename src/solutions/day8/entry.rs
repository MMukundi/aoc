...
impl FromStr for Segments {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        }).reduce(|s1,s2|s1|s2).map_or(Err(()), |s|Ok(s))
    }
}
    ...
    pub fn restrict(&mut self, index:usize,segments:&Segments){
        if self.could_be[index].count_active()!=1 {
            ...
            if self.could_be[index].count_active()==1 {
                ...
                (0..7).filter(|i|*i!=index).for_each(|i|self.restrict(i, &locked_in));
            }   
    }
    }    
    pub fn restrict_to(&mut self, expected:&Segments,found:&Segments){
        expected.iter_all().enumerate().filter(|p|p.1.1).map(|p|p.0).for_each(|i|self.restrict(i, found));
    }
    // fn adapt(&mut self){
...
impl FromStr for Entry {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut entry = Entry { signal_patterns:Default::default(), outputs:Default::default(), could_be:[Segments::ALL;7] };
        let mut entry_str = s.split(" | ");
        entry_str.next().map(|s|{
            let pattern_iter = s.split_whitespace();
            entry.signal_patterns.iter_mut().zip(pattern_iter).for_each(|(dest,src)|*dest=src.parse::<Segments>().unwrap());
        });
        entry_str.next().map(|s|{
            let outputs_iter = s.split_whitespace();
            entry.outputs.iter_mut().zip(outputs_iter).for_each(|(dest,src)|*dest=src.parse::<Segments>().unwrap());
        });
        Ok(entry)
    }
}